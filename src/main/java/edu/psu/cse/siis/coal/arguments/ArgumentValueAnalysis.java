...
public abstract class ArgumentValueAnalysis {
   ...
   */
  public List<Object> computeArgumentValues(Argument argument, Unit callSite) {
    if (argument.getArgnum() == null) {
      return null;
    }
    if (AnalysisParameters.v().useShimple()) {
      // Shimple is not supported.
      return Collections.singletonList((Object) getTopValue());
    } else {
   ...
   */
  public abstract List<Object> computeVariableValues(Value value, Stmt callSite);
   ...
   */
  public abstract List<Object> computeInlineArgumentValues(String[] inlineValues);
